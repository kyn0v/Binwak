...
> ⚠️ 若 shell 设了全局代理（`HTTP_PROXY`/`ALL_PROXY` 等，常见于 V2Ray），automator 连本地
> IDE 的 `127.0.0.1:9420` 会被代理劫持而**卡在 `RUNS` 不动**。`test:e2e:*` 脚本已内置
> `NO_PROXY=127.0.0.1,localhost` 并清空代理变量来规避；若仍卡，手动开 IDE 导入对应产物
> 目录后再跑。

> ⚠️ `jest.e2e.config.js` 用 `teardown: 'disconnect'` 跑完**故意留着 IDE**，因此实例会一直占着
> 自动化端口 `9420`；下次 `launch: true` 起的第二个实例绑不上 9420 会再次**卡在 `RUNS`**（崩溃/
> 中断的运行也会留下同样的残留）。`pretest:e2e:mp`（`node tests/e2e/free-port.js 9420`）会在每次
> 运行前自动 `cli quit` 关掉整个 DevTools 并兜底 SIGKILL 仍占 9420 的进程，自动打破死锁——一般无需
> 手动清理。改了 `mp-weixin.port` 记得同步该脚本参数。

**这套 e2e 只在本地手动跑**（CI 只跑单元测试 + 生产构建，不含本套件）。

每次运行（**无需先手动 build**）：

```bash
npm run test:e2e:mp       # automator 自己编译并启动 DevTools 自动化跑用例
```

> 💡 配置用 `compile: true`：automator 会自己跑 `dev:mp-weixin` 从源码新鲜编译，因此**不需要**
> 提前 `npm run build:mp-weixin`，改完代码直接跑即可。
>
> 🔑 **为什么必须 `compile: true`**：早期用过 `compile: false` + 预构建产物，但普通
> `uni build -p mp-weixin` **不会注入** automator 运行时 hook `wx.$$initRuntimeAutomator`，
> 导致每次都报 `wx.$$initRuntimeAutomator not exists`（运行时找不到该方法）。让 automator
> 自己编译才会注入这个 hook 并正确挂到 `wx` 上。若哪天又见到这个报错，先确认 config 仍是
> `compile: true`，并清掉 DevTools 编译缓存
> （`~/Library/Application Support/微信开发者工具/<profile>/WeappCache/WeappCompileCache/*`）。
>
> ⚠️ 另外 DevTools 首次被自动化控制时可能弹「信任项目 / 允许自动化」3 秒确认框，来不及点会导致
> 运行时起不来。可在 `cli auto` 传 `--trust-project` 跳过，或保持 IDE 已授权状态。
...
```bash
npm run test:e2e:mp -- visual.e2e -u   # -u 重写基准图（automator 自动编译）
```
